...
export default function SearchResultsPage() {
  ...
  const sortedCandidates = useMemo(() => {
    return [...candidateCoins].sort((a, b) => {
      if (sort === 'score') {
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <p className="text-sm text-stone-500">
          Showing {sortedCandidates.length} candidates. Highest similarity score currently{' '}
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        {sortedCandidates.map((candidate) => {
          ...
          return (
            <article key={candidate.id} className="flex flex-col rounded-2xl border border-stone-200 bg-white/80 shadow-card transition hover:border-gold-300">
              ...
              <div className="flex gap-3 border-t border-stone-200 px-6 py-4">
                ...
                <button
                  ...
                  onClick={() => museumCoin && navigate(`/coin/${museumCoin.coin_id}`)}
                  disabled={!museumCoin}
                >
                  View museum record
                </button>
